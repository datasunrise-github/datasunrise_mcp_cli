    ...
    if (value !== undefined) {
      if (paramDef.type === 'boolean') {
        ...
        const needsQuoting = typeof value === 'string' && (
            value.includes(' ') || 
            (paramDef.cliName === '-addMembers' && value.includes(',')) ||
            (paramDef.cliName === '-removeMembers' && value.includes(','))
        );
...
function validateInputArgs(commandDef: CliCommand, inputArgs: Record<string, any>): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  // host_add has mutually exclusive parameter groups.
  // This basic validation can be expanded.
  if (commandDef.toolName === 'host_add') {
    const types = [
      inputArgs.host, 
      inputArgs.netIPv4, 
      inputArgs.netIPv6, 
    ...
    if (types > 1) {
      errors.push("For host_add, only one type of host definition (single, IPv4 net, IPv6 net, IPv4 range, IPv6 range) can be used at a time.");
    }
    if (inputArgs.netIPv4 && !inputArgs.netMaskV4) errors.push("netIPv4 requires netMaskV4.");
    if (inputArgs.startIPv4 && !inputArgs.endIPv4) errors.push("startIPv4 requires endIPv4.");
    if (inputArgs.startIPv6 && !inputArgs.endIPv6) errors.push("startIPv6 requires endIPv6.");
  }
  if (commandDef.dependencies) {
    // General dependency logic if added to CliCommand
  }
  return { valid: errors.length === 0, errors };
}

async function runTests() {
  const testCasesPath = path.join(__dirname, 'host_command_test_cases.json'); // Adjusted path
  const testCases: TestCase[] = JSON.parse(fs.readFileSync(testCasesPath, 'utf-8'));
  
  const outputLines: string[] = [];
  let allTestsPassed = true;

  outputLines.push("Host Command Test Results:"); // Adjusted title
  outputLines.push("=========================");
  ...
  for (const tc of testCases) {
    ...
    for (const expectedParam of tc.expectedCliParams) {
      let found = false;
      // More robust check for parameters and their values, especially for quoted strings
      if (generatedCli.includes(expectedParam)) {
          found = true;
      } else {
          // Try splitting and checking if the parameter flag and its immediate next part match
          ...
          if (paramIndex !== -1 && expectedParts.length > 1) {
              if (paramIndex + 1 < cliParts.length && cliParts[paramIndex + 1] === expectedParts[1]) {
                  found = true;
              }
          }
      }
  ...
  const outputPath = path.join(__dirname, 'generated_host_commands.txt'); // Adjusted output file
...
runTests().catch(error => {
  console.error("Error running Host command tests:", error); // Adjusted error message
  process.exit(1);
});
