...
interface TestCase {
  ...
  implicitCliParams?: string[]; // For params like -serverType added by logic
  expectedValidationError?: boolean; 
}

function buildCliCommandString(commandDef: CliCommand, inputArgs: Record<string, any>, implicitParams: string[] = []): string {
  let cliString = commandDef.baseCommand;
  const appliedParams: string[] = [...implicitParams]; // Start with implicit params
    ...
    if (value !== undefined) {
      if (paramDef.type === 'boolean') {
        if (value === true) {
          ...
          // Omit if false and default was true (e.g. -certificate for server_add_smtp)
        }
      } else {
  ...
  // Filter out duplicates that might arise if an implicit param is also in commandDef.params (though unlikely with current structure)
  const uniqueAppliedParams = Array.from(new Set(appliedParams));
  return `${cliString} ${uniqueAppliedParams.join(' ')}`.trim();
}

function validateInputArgs(commandDef: CliCommand, inputArgs: Record<string, any>): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  // Server commands are relatively simple in current definitions.
  if (commandDef.dependencies) {
    errors.push("Dependency checking for server commands not fully implemented in this test script yet.");
  }
  return { valid: errors.length === 0, errors };
}

async function runTests() {
  const testCasesPath = path.join(__dirname, 'server_command_test_cases.json'); // Adjusted path
  const testCases: TestCase[] = JSON.parse(fs.readFileSync(testCasesPath, 'utf-8'));
  
  const outputLines: string[] = [];
  let allTestsPassed = true;

  outputLines.push("Server Command Test Results:"); // Adjusted title
  outputLines.push("==========================");
  ...
  for (const tc of testCases) {
    ...
    const generatedCli = buildCliCommandString(commandDef, tc.inputArgs, tc.implicitCliParams);
    outputLines.push(`Generated CLI: ${generatedCli}`);

    let testPassed = true;

    // Check for implicit params first, as they are added directly
    if(tc.implicitCliParams){
        for (const implicitParam of tc.implicitCliParams) {
            if (generatedCli.includes(implicitParam)) {
                outputLines.push(`  PASS: Implicit param "${implicitParam}" found.`);
            } else {
                outputLines.push(`  FAIL: Implicit param "${implicitParam}" NOT FOUND.`);
                testPassed = false;
            }
        }
    }
  ...
  const outputPath = path.join(__dirname, 'generated_server_commands.txt'); // Adjusted output file
...
runTests().catch(error => {
  console.error("Error running Server command tests:", error); // Adjusted error message
  process.exit(1);
});
