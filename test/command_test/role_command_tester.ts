  ...
  for (const paramDef of commandDef.params) {
    ...
    if (value !== undefined) {
      if (paramDef.type === 'boolean') {
        ...
        // or is a groupDN which can have complex structures.
        const needsQuoting = typeof value === 'string' && (
            value.includes(' ') || 
            value.includes(',') || // For action lists
            paramDef.cliName === '-groupDN'
        );
...
function validateInputArgs(commandDef: CliCommand, inputArgs: Record<string, any>): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  // Role commands are relatively simple in current definitions.
  if (commandDef.dependencies) {
    errors.push("Dependency checking for role commands not fully implemented in this test script yet.");
  }
  return { valid: errors.length === 0, errors };
}

async function runTests() {
  const testCasesPath = path.join(__dirname, 'role_command_test_cases.json'); // Adjusted path
  const testCases: TestCase[] = JSON.parse(fs.readFileSync(testCasesPath, 'utf-8'));
  
  const outputLines: string[] = [];
  let allTestsPassed = true;

  outputLines.push("Role Command Test Results:"); // Adjusted title
  outputLines.push("==========================");
  ...
  const outputPath = path.join(__dirname, 'generated_role_commands.txt'); // Adjusted output file
...
runTests().catch(error => {
  console.error("Error running Role command tests:", error); // Adjusted error message
  process.exit(1);
});
