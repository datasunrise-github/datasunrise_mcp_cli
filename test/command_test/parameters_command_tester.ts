...
interface TestCase {
  ...
  baseCommandOverride?: string; // To handle internal function "baseCommands"
}

function buildCliCommandString(commandDef: CliCommand, inputArgs: Record<string, any>, baseCommandOverride?: string): string {
  let cliString = baseCommandOverride || commandDef.baseCommand; // Use override if provided
  ...
  for (const paramDef of commandDef.params) {
    ...
    if (value !== undefined) {
      if (paramDef.type === 'boolean') {
        ...
        // Quote if it's a string and contains spaces or is a JSON string
        const needsQuoting = typeof value === 'string' && 
                             (value.includes(' ') || paramDef.name === 'parameters'); // Special quoting for JSON string
...
function validateInputArgs(commandDef: CliCommand, inputArgs: Record<string, any>): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  // Parameters commands are simple for CLI generation purposes.
  if (commandDef.dependencies) {
    errors.push("Dependency checking for parameters commands not fully implemented in this test script yet.");
  }
  return { valid: errors.length === 0, errors };
}

async function runTests() {
  const testCasesPath = path.join(__dirname, 'parameters_command_test_cases.json'); // Adjusted path
  const testCases: TestCase[] = JSON.parse(fs.readFileSync(testCasesPath, 'utf-8'));
  
  const outputLines: string[] = [];
  let allTestsPassed = true;

  outputLines.push("Parameters Command Test Results:"); // Adjusted title
  outputLines.push("===============================");
  ...
  for (const tc of testCases) {
    ...
    const generatedCli = buildCliCommandString(commandDef, tc.inputArgs, tc.baseCommandOverride);
  ...
  const outputPath = path.join(__dirname, 'generated_parameters_commands.txt'); // Adjusted output file
...
runTests().catch(error => {
  console.error("Error running Parameters command tests:", error); // Adjusted error message
  process.exit(1);
});
