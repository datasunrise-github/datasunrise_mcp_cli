  ...
  for (const paramDef of commandDef.params) {
    ...
    if (value !== undefined) {
      if (paramDef.type === 'boolean') {
        ...
        // Quote if it's a string and contains spaces or other CLI-sensitive characters
        const needsQuoting = typeof value === 'string' && (
            value.includes(' ') || 
            value.includes('(') || value.includes(')') || // Example: paths with parentheses
            value.includes("'") || value.includes('"') // If the value itself contains quotes
        );

        if (needsQuoting) {
            // Avoid double-quoting if already quoted by test case for some reason
            if (!((value.startsWith('"') && value.endsWith('"')) || (value.startsWith("'") && value.endsWith("'")))) {
                 // Escape internal double quotes before surrounding with new double quotes
...
function validateInputArgs(commandDef: CliCommand, inputArgs: Record<string, any>): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  // Import commands are simple and don't have complex dependencies defined.
  if (commandDef.dependencies) {
    errors.push("Dependency checking for import commands not fully implemented in this test script yet.");
  }
  return { valid: errors.length === 0, errors };
}

async function runTests() {
  const testCasesPath = path.join(__dirname, 'import_command_test_cases.json'); // Adjusted path
  const testCases: TestCase[] = JSON.parse(fs.readFileSync(testCasesPath, 'utf-8'));
  
  const outputLines: string[] = [];
  let allTestsPassed = true;

  outputLines.push("Import Command Test Results:"); // Adjusted title
  outputLines.push("===========================");
  ...
  for (const tc of testCases) {
    ...
    for (const expectedParam of tc.expectedCliParams) {
      // A more direct check for fileName as it's the only param and can contain special chars
    ...
    if (tc.absentCliParams) { // Should not be applicable for import commands
  ...
  const outputPath = path.join(__dirname, 'generated_import_commands.txt'); // Adjusted output file
...
runTests().catch(error => {
  console.error("Error running Import command tests:", error); // Adjusted error message
  process.exit(1);
});
