  ...
  for (const paramDef of commandDef.params) {
    ...
    if (value !== undefined) {
      if (paramDef.type === 'boolean') {
        ...
        // Quote if it's a string and contains spaces or special characters like ';' or '=' for intervals
...
function validateInputArgs(commandDef: CliCommand, inputArgs: Record<string, any>): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  // Schedule commands are relatively simple in current definitions.
  if (commandDef.dependencies) {
    errors.push("Dependency checking for schedule commands not fully implemented in this test script yet.");
  }
  return { valid: errors.length === 0, errors };
}

async function runTests() {
  const testCasesPath = path.join(__dirname, 'schedule_command_test_cases.json'); // Adjusted path
  const testCases: TestCase[] = JSON.parse(fs.readFileSync(testCasesPath, 'utf-8'));
  
  const outputLines: string[] = [];
  let allTestsPassed = true;

  outputLines.push("Schedule Command Test Results:"); // Adjusted title
  outputLines.push("============================");
  ...
  const outputPath = path.join(__dirname, 'generated_schedule_commands.txt'); // Adjusted output file
...
runTests().catch(error => {
  console.error("Error running Schedule command tests:", error); // Adjusted error message
  process.exit(1);
});
